...
class GumbelEncoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.sample_fwards = params.forwards_inf
        if self.sample_fwards:
            self.fwards = nn.GRU(params.n_latent, params.n_hidden,
                                 params.n_layers, dropout=params.dropout,
                                 batch_first=True)
            self.init_fwds = lambda b: init_hidden(
                nn.GRU, params.n_layers, b, params.n_hidden, torch.float32,
                torch.device('cpu'))
    ...
    def forward(self, tau, x, h=None):
        if self.sample_fwards:
            # Preallocate the tensors for q_z, and z_hat (the sampled values of z)
            logits_q_z = Variable(x.new(*x.shape[:2], self.n_latent))
            z = Variable(x.new(*x.shape[:2], self.n_latent))
            h_fwards, h_bwards = h

            bwards_output, _, h = self.inference((x, h_bwards))

            # # Sample from the initial distribution and do the first step
            init_z_sample = gumbel_softmax(self.init_z, tau, hard=False,
                                           shape=(x.shape[0], 1, 1))
            fwards_output, h_fwards = self.fwards(init_z_sample, h_fwards)
            q_z_1_logits = self.combiner(fwards_output, bwards_output[:, 0].unsqueeze(1))
            logits_q_z[:, 0] = q_z_1_logits.squeeze(1)

            # q_z initially contains the logits of the categorical distribution
            # we are trying to approximate using the Gumbel Softmax trick
            z_prev = gumbel_softmax(q_z_1_logits, tau, hard=False)
            z[:, 0] = z_prev.squeeze(1)

            for t in range(1, x.shape[1]):
                fwards_output, h_fwards = self.fwards(z_prev, h_fwards)
                q_z_t_logits = self.combiner(fwards_output, bwards_output[:, t].unsqueeze(1))
                z_prev = gumbel_softmax(q_z_t_logits, tau, hard=False)

                z[:, t] = z_prev.squeeze(1)
                logits_q_z[:, t] = q_z_t_logits.squeeze(1)
            ...
            q_z = F.softmax(logits_q_z, dim=-1)
            z = gumbel_softmax(logits_q_z, tau, hard=False)
        return logits_q_z, {'z': q_z}, {'z': z}, h

    def init_hidden(self, batch_size):
        if type(self.inference) == RNNDistribution:
            h_bwards = self.inference.init_hidden(batch_size)
            if self.sample_fwards:
                h_fwards = self.init_fwds(batch_size)
                return h_fwards, h_bwards
            else:
                return h_bwards
        else:
            return None
...
class GumbelDecoder(nn.Module):
    ...
    def forward(self, E, h=None):
        ...
        # Covariances
        c = self.covariance()
        # Ensure is positive definiteimp
...
class GumbelRNNDecoder(nn.Module):
    def __init__(self, params):
        super(GumbelRNNDecoder, self).__init__()
        ...
        self.obs = RNNDistribution(params)
